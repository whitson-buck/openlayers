...
var collectCopyTargets = async (root, targets, flatten) => {
  ...
  for (const { src, dest, rename, transform, preserveTimestamps } of targets) {
    ...
    for (const matchedPath of matchedPaths) {
      ...
      copyTargets.push({
        src: matchedPath,
        dest: import_node_path.default.join(destDir, rename ?? base),
        transform,
...
async function transformCopy(transform, src, dest) {
  const content = await import_fs_extra.default.readFile(src, "utf8");
...
var copyAll = async (rootSrc, rootDest, targets, flatten) => {
  ...
  for (const { src, dest, transform, preserveTimestamps } of copyTargets) {
    ...
    if (transform) {
      ...
      await import_fs_extra.default.copy(resolvedSrc, resolvedDest, { preserveTimestamps });
    }
  }
...
function getTransformHeaders(name, content) {
  let ctype = lookup(name) || "";
  if (ctype === "text/html")
    ctype += ";charset=utf-8";
  const headers = {
